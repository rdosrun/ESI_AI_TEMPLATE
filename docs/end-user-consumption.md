...
The starter kit should let users consume AI through familiar business tools instead of asking them to learn a new standalone application. The FastMCP server remains the control layer, while Microsoft 365 and agentic platforms become user-facing channels.
...
| Microsoft Teams | Users ask questions in a team channel or personal assistant experience. | An MCP-capable Teams agent calls `ask_question`. |
...
| Power Automate | Users trigger AI steps inside business workflows. | A connector or MCP bridge calls the governed server tools. |
| Agentic orchestrator such as Hermes | An agent plans multi-step work, retrieves context, selects skills, calls tools, and escalates decisions. | Hermes connects to `/mcp` and discovers the governed tools directly. |
...
6. The user-facing channel or agent calls the FastMCP `ask_question` tool.
...
3. Hermes calls `ask_question` with the question and workflow context.
4. Hermes can call `search_skills` to find the right approved skill or skill group.
